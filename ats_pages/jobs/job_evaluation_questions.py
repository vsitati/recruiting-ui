...
class Elements:
    select_checkbox = (By.NAME, "evalquestionid")
    ...
    link_text_elem = (By.LINK_TEXT, JobData.job_data.get("cqe"))
...
class JobEvaluationQuestions(Common, Elements):
    ...
    def select_cqe(self):
        # open the link
        elm_link = self.driver.find_element_by_locator(self.link_text_elem)
        self.do_click(elm_link)
        # check a checkbox
        elm_checkbox = self.driver.find_element(locate_with(*self.select_checkbox).below(elm_link))
        ...
        elm = self.driver.find_element(locate_with(*self.return_to_job_lists_btn).to_left_of(elm_btn))
        self.do_click(elm)
